    ...
    for(var i = 0; i < items.length; i++) {
        ...
        if(items[i].author) {
            for(var y = 0; y < items[i].author.length; y++) {
                var textAuthor = undefined;

                if(items[i].author[y].email)
                    textAuthor = items[i].author[y].email;

                if(items[i].author[y].name) {
                    if(textAuthor === undefined) {
                        textAuthor = items[i].author[y].name;
                    } else {
                        textAuthor += ' (' + items[i].author[y].name + ')';
                    }
                }

                item.push({ author: textAuthor });
            }
        }

        if(items[i].image) {
          var _attr = { url: items[i].image };
          var type = mime.lookup(items[i].image);
          if (type)
            _attr.type = type;
          item.push({ enclosure: [{ _attr: _attr }] });
        }
